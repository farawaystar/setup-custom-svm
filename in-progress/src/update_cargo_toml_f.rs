...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let checkout_path = format!("./output/filter-repo-command.sh");
    ...
    let mut checked_out: Vec<String> = checkout_command
      ...
      .collect();
    println!("Checked out folders: {:?}", checked_out);
        ...
        if let Some(members) = workspace.get("members").and_then(|m| m.as_array()) {
            let filtered_members: Vec<Value> = members
                ...
                .filter_map(|member| {
                    ...
                    if checked_out.iter().any(|folder| member_str.starts_with(folder)) {
